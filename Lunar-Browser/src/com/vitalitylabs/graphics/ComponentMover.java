...
public class ComponentMover extends MouseAdapter {
	...
	private int dragRegionX = 0;
	private int dragRegionY = 0;
	private int dragRegionWidth = 0;
	private int dragRegionHeight = 0;
	...
	public void setDraggableRegion(int x, int y, int width, int height) {
		dragRegionX = x;
		dragRegionY = y;
		dragRegionWidth = width;
	...
	private boolean isMouseLocationInBounds() {
		int cursorX = (MouseInfo.getPointerInfo().getLocation().x) - source.getX();
		int cursorY = (MouseInfo.getPointerInfo().getLocation().y) - source.getY();
		
		

		if ((cursorX >= dragRegionX) && (cursorY >= dragRegionY)) {
			if (cursorX <= (dragRegionX + dragRegionWidth) && (cursorY <= (dragRegionY + dragRegionHeight))) {
				return true;
			}
		}
		return false;
